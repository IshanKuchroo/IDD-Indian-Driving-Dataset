...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.linear3_bn = nn.BatchNorm1d(128)

        self.linear4 = nn.Linear(128, OUTPUTS_a)
    ...
    def forward(self, x):
        ...
        x = self.linear2_bn(self.act1(self.linear2(x)))
        x = self.linear3_bn(self.act1(self.linear3(x)))
        return self.act2(self.linear4(x))
...
def train_and_test(train_ds, valid_ds, list_of_metrics, list_of_agg, save_on, pretrained=False):
        ...
        training_stats.append(
            {
                ...
                'Train F1 Weighted': train_f1weighted[epoch],
                'Valid F1 Weighted': val_f1weighted[epoch]
            }
        )
    ...
    plt.plot(df_stats['Train F1 Weighted'], marker='*', color='blue', label="Training")
    plt.plot(df_stats['Valid F1 Weighted'], marker='*', color='red', label="Validation")

    # Label the plot.
    plt.title("Training & Validation F1 Weighted")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.xticks(lst)
...
if __name__ == '__main__':
    ...
    list_of_metrics = ['acc','f1_weighted','hlm']
    list_of_agg = ['sum']
